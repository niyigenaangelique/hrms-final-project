<div class="pe-root">
    ...
    @if(session()->has('error'))
        <div class="pe-flash pe-flash-err" style="margin-bottom: 20px; word-wrap: break-word;">
            <svg viewBox="0 0 24 24">
                            ...
                            <td>
                                @if($row->overtime_hours_worked > 0)
                                    <div style="font-weight:700;color:var(--indigo);">
                                        {{ number_format($row->overtime_hours_worked, 1) }} hrs
                                    </div>
                    ...
                    <div class="pe-view-grid">
                        ...
                        <div class="pe-view-row">
                            ...
                            <div class="pe-view-val" style="font-size:12.5px;">
                                {{ \Carbon\Carbon::parse($viewRecord->created_at)->format('M d, Y · H:i') }}
                            </div>
                        </div>
                        <div class="pe-view-row">
                            <div class="pe-view-lbl">Last Updated</div>
                            <div class="pe-view-val" style="font-size:12.5px;">
                                {{ \Carbon\Carbon::parse($viewRecord->updated_at)->format('M d, Y · H:i') }}
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        Livewire.hook('component.initialized', (component) => {
            console.log('Payroll Entry Component Initialized:', component.name);
        });

        Livewire.hook('request.failed', (request, response) => {
        ...
        // Log form submission attempts - Only for payroll entry form
        document.addEventListener('click', function (e) {
            // Check if we're on the payroll entry page and save button was clicked
            if (e.target.closest('[wire\\:click="save"]') && window.location.pathname.includes('payroll-entry')) {
                console.log('Payroll Entry Save button clicked, checking form data...');

                // Get form values
                const employeeId = document.querySelector('[wire\\:model="employeeId"]')?.value;
                const payrollMonthId = document.querySelector('[wire\\:model="payrollMonthId"]')?.value;
                const dailyRate = document.querySelector('[wire\\:model="dailyRate"]')?.value;
                const workDays = document.querySelector('[wire\\:model="workDays"]')?.value;
                ...
                if (!payrollMonthId) {
                    console.error('Payroll month not selected');
                }
                if (!dailyRate || isNaN(dailyRate)) {
                    console.error('Invalid daily rate');
                }
                if (!workDays || isNaN(workDays)) {
                    console.error('Invalid work days');
                }
            }
        });
    });
