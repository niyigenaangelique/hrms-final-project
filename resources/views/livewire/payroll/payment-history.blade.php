<div class="ph-root">
    <style>
        ...
        .ph-hero-cover {
            height: 72px;
            background: linear-gradient(118deg, #1A3FA8 0%, #2755CC 36%, #3B6FE8 66%, #12B76A 100%);
            position: relative;
            overflow: hidden;
        }

        .ph-hero-cover::before {
            content: '';
            position: absolute;
            top: -40px;
            right: 80px;
        ...
        .ph-hero-icon {
            ...
            height: 52px;
            border-radius: 14px;
            background: linear-gradient(135deg, var(--green), #0DD47A);
            border: 3px solid var(--white);
            box-shadow: 0 4px 14px rgba(18, 183, 106, 0.30);
            display: flex;
            align-items: center;
        ...
        .ph-t-blue .ph-tile-bar {
            background: var(--blue);
        }

        .ph-t-blue .ph-tile-icon {
            background: var(--blue-lt);
        }

        .ph-t-blue .ph-tile-icon svg {
            stroke: var(--blue);
        }

        .ph-t-blue .ph-tile-val {
            color: var(--blue-2);
        }

        .ph-t-green .ph-tile-bar {
            background: var(--green);
        }

        .ph-t-green .ph-tile-icon {
            background: var(--green-lt);
        }

        .ph-t-green .ph-tile-icon svg {
            stroke: var(--green);
        }

        .ph-t-green .ph-tile-val {
            color: var(--green);
        }

        .ph-t-amber .ph-tile-bar {
            background: var(--amber);
        }

        .ph-t-amber .ph-tile-icon {
            background: var(--amber-lt);
        }

        .ph-t-amber .ph-tile-icon svg {
            stroke: var(--amber);
        }

        .ph-t-amber .ph-tile-val {
            color: var(--amber);
        }

        .ph-t-teal .ph-tile-bar {
            background: var(--teal);
        }

        .ph-t-teal .ph-tile-icon {
            background: var(--teal-lt);
        }

        .ph-t-teal .ph-tile-icon svg {
            stroke: var(--teal);
        }

        .ph-t-teal .ph-tile-val {
            color: var(--teal);
        }
        ...
        .ph-modal-hd {
            background: linear-gradient(105deg, var(--blue-3), var(--blue) 60%, #12B76A 130%);
            padding: 20px 24px;
            display: flex;
            align-items: center;
            justify-content: space-between;
    ...
    <div class="ph-tiles">
        <div class="ph-tile ph-t-blue">
            ...
            <div class="ph-tile-icon"><svg viewBox="0 0 24 24">
                    <rect x="2" y="5" width="20" height="14" rx="2" />
                    <line x1="2" y1="10" x2="22" y2="10" />
                </svg></div>
            <div>
    ...
    <div class="ph-card">
                ...
                <tbody>
                    @forelse($records as $row)
                        ...
                        <tr>
                            ...
                            <td
                                style="font-size:12px;color:var(--ink3);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                            ...
                            <td>
                                <div class="ph-actions">
                                    <button class="ph-btn ph-btn-ghost ph-btn-sm" wire:click="openView({{ $row->id }})"
                                        title="View">
                                        <svg viewBox="0 0 24 24">
                                    ...
                                    <button class="ph-btn ph-btn-outline ph-btn-sm" wire:click="openEdit({{ $row->id }})"
                                        title="Edit">
                                        <svg viewBox="0 0 24 24">
                                    ...
                                    @endif
                                    <button class="ph-btn ph-btn-danger ph-btn-sm"
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="ph-empty">
                                    ...
                                    <div class="ph-empty-sub">
                                        {{ $search || $filterStatus || $filterApproval || $filterMethod ? 'Try adjusting your filters.' : 'Create the first payment record to get started.' }}
                                    </div>
        ...
        @if($records->hasPages())
            <div class="ph-pager">
                <div class="ph-pager-info">Showing {{ $records->firstItem() }}–{{ $records->lastItem() }} of
                    {{ $records->total() }}</div>
                {{ $records->links() }}
            </div>
        @endif
            ...
            <div class="ph-modal ph-modal-lg">
                ...
                <div class="ph-modal-hd">
                    <div class="ph-modal-hd-left">
                        ...
                        <div>
                            <div class="ph-modal-title">{{ $editingId ? 'Edit Payment Record' : 'New Payment Record' }}
                            </div>
                            <div class="ph-modal-sub">
                                {{ $editingId ? 'Update payment details' : 'Record a new employee payment' }}</div>
                        </div>
                    </div>
                ...
                <div class="ph-modal-body">
                    ...
                    <div class="ph-tab-panel active" id="phTab-basic">
                        <div class="ph-grid2">
                            <div class="ph-field">
                                ...
                                <input type="text" wire:model="code" placeholder="PAY-00001"
                    ...
                    <div class="ph-tab-panel" id="phTab-banking">
                        <div
                            style="background:var(--blue-lt);border:1px solid var(--blue-brd);border-radius:var(--r);padding:11px 14px;font-size:12.5px;color:var(--blue-2);font-weight:600;">
                            ...
                            <div class="ph-field">
                                ...
                                <select wire:model="payrollEntryId">
                                    ...
                                    @foreach($payrollEntries as $pe)
                                        ...
                                        @endphp
                                        <option value="{{ $pe['id'] }}">{{ $pe['code'] ?? '#' . $pe['id'] }} — {{ trim($peEmp) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="ph-modal-body" style="padding-top:20px;">
                    ...
                    <div class="ph-view-grid">
                        ...
                        <div class="ph-view-row">
                            ...
                            <div class="ph-view-val"
                                style="font-family:'Sora',sans-serif;font-size:18px;color:var(--green);">
                                {{ number_format($viewRecord->amount_paid, 2) }}
                                <span
                                    style="font-size:12px;color:var(--ink4);font-family:'DM Sans',sans-serif;">{{ $viewRecord->currency }}</span>
                            </div>
                        </div>
                        <div class="ph-view-row">
                            <div class="ph-view-lbl">Payment Date</div>
                            <div class="ph-view-val">
                                {{ \Carbon\Carbon::parse($viewRecord->payment_date)->format('l, M d, Y') }}</div>
                        </div>
                        <div class="ph-view-row">
                            <div class="ph-view-lbl">Payment Method</div>
                            <div class="ph-view-val">
                                {{ $paymentMethods[$vMethod] ?? ucfirst(str_replace('_', ' ', $vMethod)) }}</div>
                        </div>
                        <div class="ph-view-row">
                        ...
                        <div class="ph-view-row">
                            ...
                            <div class="ph-view-val">
                                {{ $viewRecord->payrollEntry?->code ?? ($viewRecord->payroll_entry_id ? '#' . $viewRecord->payroll_entry_id : '—') }}
                            </div>
                        </div>
                        <div class="ph-view-row">
                        ...
                        @if($viewRecord->notes)
                            <div class="ph-view-row full">
                                ...
                                <div class="ph-view-val" style="font-size:13px;color:var(--ink3);line-height:1.6;">
                                    {{ $viewRecord->notes }}</div>
                            </div>
                        @endif
                        <div class="ph-view-row">
                            <div class="ph-view-lbl">Created</div>
                            <div class="ph-view-val" style="font-size:12.5px;">
        ...
        <div class="ph-modal-bg" wire:click.self="cancelDelete">
            <div class="ph-modal ph-del-modal">
                <div class="ph-del-body">
                    ...
                    <div class="ph-del-sub">This will permanently remove this payment record. This action cannot be undone.
                    </div>
                </div>
