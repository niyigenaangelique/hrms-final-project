        ...
        <ul role="list">
            @forelse($this->authorizedOvertimes as $index => $snakeModel)
                <li
                    ...
                    wire:click.prevent="dispatchSelectAuthorizedOvertime('{{ $snakeModel->id }}')"
            ...
            @empty
                <li class="text-sm text-gray-500 dark:text-gray-400">No authorized_overtime found.</li>
            @endforelse
        </ul>
    </div>
    <!-- Pagination -->
    <div wire:key="authorizedOvertimes-pagination">
        {{ $this->authorizedOvertimes->links() }}
    </div>
</div>
