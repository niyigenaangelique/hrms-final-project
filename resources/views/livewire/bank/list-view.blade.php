<div
    ...
    <div wire:poll class="w-full !p-0 flex-grow max-w-md bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <ul role="list">
            @forelse($this->banks as $index => $snakeModel)
                <li
                    ...
                    wire:click.prevent="dispatchSelectBank('{{ $snakeModel->id }}')"
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0 ms-4">
                            ...
                            <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                                {{ $snakeModel->country }} - {{ $snakeModel->city }} ({{ $snakeModel->code }})
                            </p>
                        </div>
                    </div>
                </li>
            @empty
                <li class="text-sm text-gray-500 dark:text-gray-400">No bank found.</li>
            @endforelse
        </ul>
    </div>
    <!-- Pagination -->
    <div wire:key="banks-pagination">
        {{ $this->banks->links() }}
    </div>
</div>
