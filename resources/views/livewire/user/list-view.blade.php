<div
    ...
    <div wire:poll class="w-full !p-0 flex-grow max-w-md bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <ul role="list">
            @forelse($this->users as $index => $snakeModel)
                <li
                    ...
                    wire:click.prevent="dispatchSelectUser('{{ $snakeModel->id }}')"
            ...
            @empty
                <li class="text-sm text-gray-500 dark:text-gray-400">No user found.</li>
            @endforelse
        </ul>
    </div>
    <!-- Pagination -->
    <div wire:key="users-pagination">
        {{ $this->users->links() }}
    </div>
</div>
