<div
    ...
    <flux:heading size="lg" class="!text-primary-900 mx-auto">List of Contracts</flux:heading>
    ...
    <div wire:poll class="w-full !p-0 flex-grow max-w-md bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <ul role="list">
            @forelse($this->contracts as $index => $snakeModel)
                <li
                    ...
                    wire:click.prevent="dispatchSelectContract('{{ $snakeModel->id }}')"
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0 ms-4">
                            <p class="text-sm font-medium text-gray-900 truncate dark:text-white">
                                {{ $snakeModel->employee->first_name }} {{ $snakeModel->employee->last_name }}
                            </p>
                            <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                                {{ $snakeModel->position->name }}
                            </p>
                        </div>
                    </div>
                </li>
            @empty
                <li class="text-sm text-gray-500 dark:text-gray-400">No contract found.</li>
            @endforelse
        </ul>
    </div>
    <!-- Pagination -->
    <div wire:key="contracts-pagination">
