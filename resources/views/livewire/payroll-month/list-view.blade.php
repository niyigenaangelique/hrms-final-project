        ...
        <ul role="list">
            @forelse($this->payrollMonths as $index => $snakeModel)
                <li
                    ...
                    wire:click.prevent="dispatchSelectPayrollMonth('{{ $snakeModel->id }}')"
            ...
            @empty
                <li class="text-sm text-gray-500 dark:text-gray-400">No payroll_month found.</li>
            @endforelse
        </ul>
    </div>
    <!-- Pagination -->
    <div wire:key="payrollMonths-pagination">
        {{ $this->payrollMonths->links() }}
    </div>
</div>
