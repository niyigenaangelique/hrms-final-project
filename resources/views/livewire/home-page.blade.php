<div x-data class="ios-root">
    ...
    <style>
        ...
        .ios-icon-red    { background: rgba(239,68,68,0.15); } .ios-icon-red    svg { stroke: #dc2626; }
    ...
    <div class="ios-main">
        ...
        <div class="ios-topbar">
            <div>
                <h1 class="ios-greeting">HR Dashboard 👋</h1>
                <p class="ios-date">{{ now()->format('l, j F Y') }}</p>
            </div>
            <div class="ios-code-badge">{{ Auth::user()->role->value }}</div>
        </div>
        ...
        <div class="ios-stats">
            <div class="ios-stat">
                ...
                <div class="ios-stat-label">Total Employees</div>
                <div class="ios-stat-value">{{ App\Models\Employee::count() }}</div>
            </div>

            <div class="ios-stat">
                <div class="ios-stat-icon ios-icon-blue">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/></svg>
                </div>
                <div class="ios-stat-label">Active Employees</div>
                <div class="ios-stat-value">{{ App\Models\Employee::where('is_active', true)->count() }}</div>
            </div>

            <div class="ios-stat">
                <div class="ios-stat-icon ios-icon-amber">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4m0 6h.01"/><path d="M5 12h14"/><rect x="2" y="6" width="20" height="8" rx="2"/></svg>
                </div>
                <div class="ios-stat-label">Pending Leave</div>
                <div class="ios-stat-value">{{ App\Models\LeaveRequest::where('status', 'pending')->count() }}</div>
            </div>

            <div class="ios-stat">
                @if(App\Models\LeaveRequest::where('status', 'pending')->count() > 0)
                    <span class="ios-stat-dot"></span>
                @endif
                <div class="ios-stat-icon ios-icon-purple">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 5H7a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2v-2"/><path d="M5 12h14"/><rect x="2" y="6" width="20" height="8" rx="2"/></svg>
                </div>
                <div class="ios-stat-label">On Leave Today</div>
                <div class="ios-stat-value">{{ App\Models\LeaveRequest::where('status', 'approved')->whereDate('start_date', '<=', now())->whereDate('end_date', '>=', now())->count() }}</div>
            </div>
        </div>
        ...
        <div class="ios-charts">
            <div class="g-card">
                <div class="ios-card-title">Department Distribution</div>
                <div class="ios-chart-wrap"><canvas id="deptChart"></canvas></div>
            </div>
            <div class="g-card">
                <div class="ios-card-title">Leave Trends</div>
                <div class="ios-chart-wrap"><canvas id="leaveTrendsChart"></canvas></div>
            </div>
            <div class="g-card">
                <div class="ios-card-title">Attendance Rate</div>
                <div class="ios-chart-wrap"><canvas id="attendanceChart"></canvas></div>
            </div>
        </div>
        ...
        <div class="ios-bottom">
            <div class="g-card">
                <div class="ios-card-title">Recent Hires</div>
                @php
                    $recentHires = App\Models\Employee::orderBy('join_date', 'desc')->take(5)->get();
                @endphp
                @if($recentHires->count() > 0)
                    @foreach($recentHires as $hire)
                        <div class="ios-list-item">
                            <div>
                                <div class="ios-list-main">{{ $hire->first_name }} {{ $hire->last_name }}</div>
                                <div class="ios-list-sub">{{ $hire->position ?? 'No Position' }} • {{ $hire->join_date?->format('M d, Y') }}</div>
                            </div>
                            <span class="ios-badge ios-badge-green">New</span>
                        </div>
                    @endforeach
                @else
                    <p class="ios-empty">No recent hires found</p>
                @endif
            </div>

            <div class="g-card">
                <div class="ios-card-title">Recent HR Activity</div>
                @php
                    $recentActivities = [
                        ['type' => 'leave', 'description' => 'Leave request approved', 'time' => '2 hours ago'],
                        ['type' => 'employee', 'description' => 'New employee onboarded', 'time' => '5 hours ago'],
                        ['type' => 'attendance', 'description' => 'Attendance report generated', 'time' => '1 day ago'],
                        ['type' => 'payroll', 'description' => 'Payroll processed', 'time' => '2 days ago'],
                    ];
                @endphp
    ...
    <nav class="ios-nav">
        <a href="{{ route('home') }}" class="ios-nav-item active">
        ...
        <a href="{{ route('admin.employees') }}" class="ios-nav-item">
    ...
    (function () {
        function initCharts() {
            ...
            /* Department doughnut */
            @php
                $deptData = App\Models\Employee::whereNotNull('department')
                    ->where('department', '!=', '')
            ...
            new Chart(document.getElementById('deptChart'), {
                type: 'doughnut',
                data: {
                    labels: @json($deptData->pluck('department')),
                    datasets: [{
                        data: @json($deptData->pluck('count')),
                        backgroundColor: ['rgba(34,197,94,0.75)', 'rgba(245,158,11,0.75)', 'rgba(239,68,68,0.75)', 'rgba(59,130,246,0.75)', 'rgba(168,85,247,0.75)'],
                        borderColor:     ['rgba(34,197,94,0.2)',  'rgba(245,158,11,0.2)',  'rgba(239,68,68,0.2)',  'rgba(59,130,246,0.2)',  'rgba(168,85,247,0.2)'],
                        borderWidth: 3,
                        hoverOffset: 6
                    }]
                },
            ...
            @php
                ...
                for($i = 5; $i >= 0; $i--) {
                    ...
                    $approved = App\Models\LeaveRequest::where('status', 'approved')
                        ->whereMonth('start_date', $month->month)
                        ->whereYear('start_date', $month->year)
                        ->count();
                    $pending = App\Models\LeaveRequest::where('status', 'pending')
                        ->whereMonth('start_date', $month->month)
                        ->whereYear('start_date', $month->year)
                        ->count();
                    $leaveTrends[] = [
            ...
            new Chart(document.getElementById('leaveTrendsChart'), {
                type: 'line',
                data: {
                    labels: @json(array_column($leaveTrends, 'month')),
                    datasets: [
                        { label: 'Approved', data: @json(array_column($leaveTrends, 'approved')), borderColor: 'rgba(34,197,94,0.85)', backgroundColor: 'rgba(34,197,94,0.08)', borderWidth: 2, pointRadius: 4, pointBackgroundColor: 'rgba(34,197,94,0.9)', pointBorderColor: '#fff', pointBorderWidth: 2, tension: 0.45, fill: true },
                        { label: 'Pending', data: @json(array_column($leaveTrends, 'pending')), borderColor: 'rgba(245,158,11,0.85)', backgroundColor: 'rgba(245,158,11,0.08)', borderWidth: 2, pointRadius: 4, pointBackgroundColor: 'rgba(245,158,11,0.9)', pointBorderColor: '#fff', pointBorderWidth: 2, tension: 0.45, fill: true }
                    ]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { position: 'bottom', labels: { boxWidth: 8, boxHeight: 8, borderRadius: 4, useBorderRadius: true, padding: 14, font, color: tick } } },
                    scales: {
                        x: { grid: { display: false }, border: { display: false }, ticks: { color: tick, font } },
                        y: { beginAtZero: true, grid: { color: grid }, border: { display: false }, ticks: { color: tick, font } }
                    }
                }
            });
            ...
            new Chart(document.getElementById('attendanceChart'), {
                ...
                data: {
                    ...
                    datasets: [{ label: 'Attendance Rate', data: @json(array_column($attendanceData, 'rate')), backgroundColor: 'rgba(59,130,246,0.6)', borderColor: 'rgba(59,130,246,0.85)', borderWidth: 1, borderRadius: 6, borderSkipped: false, barPercentage: 0.55 }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
                    ...
                    scales: {
                        x: { grid: { display: false }, border: { display: false }, ticks: { color: tick, font } },
                        y: { beginAtZero: true, max: 100, grid: { color: grid }, border: { display: false }, ticks: { color: tick, font } }
                    }
                }
            });
        }
