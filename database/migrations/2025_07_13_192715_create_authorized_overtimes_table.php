...
return new class extends Migration
{
    ...
    {
        Schema::create('authorized_overtimes', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->string('code')->unique();
            $table->foreignUuid('employee_id')->constrained()->cascadeOnDelete();
            $table->date('start_date')->nullable();
            $table->date('end_date')->nullable();
            $table->decimal('hour_rate', 10, 2)->default(1.50);

            $table->enum('approval_status', ApprovalStatus::values())
                ->default(ApprovalStatus::Initiated)
    ...
    {
        Schema::dropIfExists('authorized_overtimes');
    }
};
