...
#[Title('TalentFlow Pro | KPIs')]
class KpiManager extends Component
{
    use WithPagination;
    ...
    // ══ FORM FIELDS ══════════════════════════════════════════
    public string $code                  = '';
    public string $kpiId                 = '';   // parent KPI reference
    ...
    protected function rules(): array
    {
        return [
            'code'                  => ['required', 'string', 'max:255',
                                        Rule::unique('kpis', 'code')->ignore($this->editingId, 'id')],
            ...
            'kpiId'                 => ['nullable', 'string'],
        ];
    }

    protected array $messages = [
        'code.required'       => 'KPI code is required.',
    ...
    private function recalculate(): void
    {
        ...
        if ($target > 0 && $actual >= 0) {
            ...
            $score = round(min($pct, 100) / 10, 2); // 0-10 scale
    ...
    private function generateCode(): string
    {
        ...
        return 'KPI-' . str_pad($n, 4, '0', STR_PAD_LEFT);
    }
    ...
    {
        try {
            $r = Kpi::findOrFail($this->deletingId);

            // Guard: block if targets exist
            $hasTargets = method_exists($r, 'targets') && $r->targets()->exists();
            if ($hasTargets) {
                session()->flash('error', 'Cannot delete: this KPI has linked targets. Remove targets first.');
                $this->cancelDelete();
                return;
            }
    ...
    public function save(): void
    {
        ...
        try {
            ...
            if ($this->editingId) {
                ...
                // UUID handled by model boot or DB default
                Kpi::create($data);
    ...
    public function render()
    {
        ...
        // Parent KPI list (other KPIs for linking)
        $parentKpis = [];
        try {
            $parentKpis = Kpi::when($this->editingId, fn($q) => $q->where('id', '!=', $this->editingId))
                ->orderBy('code')->get(['id', 'code'])->toArray();
        } catch (\Exception) {}

        return view('livewire.performance.kpi-manager', [
            'records'         => $records,
